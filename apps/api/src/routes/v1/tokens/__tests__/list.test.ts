...
describe('GET /v1/tokens - Token Listing', () => {
  ...
  describe('Successful Token Listing', () => {
    it('should return empty array when user has no tokens', async () => {
      ...
      const response = await makeAuthenticatedRequest(app, 'GET', '/v1/tokens', sessionToken);
    ...
    it('should mask token values correctly using last4', async () => {
      ...
      // Create a token
      const { tokenRecord, token: plaintextToken } = await createApiToken(user.id, {
        name: 'Test Token',
        scopes: ['read:transactions'],
      });
  ...
  describe('Access Control', () => {
    it('should only return tokens belonging to authenticated user', async () => {
      ...
      const response1 = await makeAuthenticatedRequest(app, 'GET', '/v1/tokens', sessionToken1);
      ...
      const response2 = await makeAuthenticatedRequest(app, 'GET', '/v1/tokens', sessionToken2);
  ...
  describe('Authentication Requirements', () => {
    ...
    it('should reject invalid session token', async () => {
      ...
      const response = await makeAuthenticatedRequest(app, 'GET', '/v1/tokens', 'invalid-token');

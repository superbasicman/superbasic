...
- **Secure by design**: Auth-core OAuth 2.1/OIDC with short-lived tokens, Upstash Redis rate limiting (10 req/min on auth endpoints), structured audit logging
...
- Authentication secrets (auth-core signing keys and token hash key)
...
# - AUTH_CORE_JWT_SECRET / TOKEN_HASH_KEY (auth-core signing + hash keys)
# - UPSTASH_REDIS_REST_URL (optional, for rate limiting)
# - UPSTASH_REDIS_REST_TOKEN (optional, for rate limiting)
# - GOOGLE_CLIENT_ID (optional, for Google OAuth via auth-core)
# - GOOGLE_CLIENT_SECRET (optional, for Google OAuth via auth-core)
...
Auth-core OAuth 2.1/OIDC service for token issuance/validation (access tokens, refresh tokens, PAT hashing, AuthContext).
...
- First-party auth-core credential flow (email and password)
- Passwords hashed with bcrypt (cost factor 10)
- Short-lived access tokens + rotated refresh tokens (HttpOnly cookies)

### OAuth Providers (Optional)

Configure Google OIDC for auth-core:

**Google OAuth:**

1. Create OAuth app in [Google Cloud Console](https://console.cloud.google.com/apis/credentials)
2. Add the authorized redirect URI from `docs/auth-migration/end-auth-goal.md` (auth-core authorize callback)
3. Copy client ID and secret to `.env.local` (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`)
4. See [OAuth Setup Guide](docs/oauth-setup-guide.md) for auth-core configuration details
...
- ✅ Auth-core OAuth 2.1/OIDC with short-lived access tokens + rotated refresh tokens (HttpOnly cookies)
- ✅ Google OIDC integration with PKCE and CSRF protection
...
- 🛠️ Row-level security policies in Postgres - Phase 6
- 🔮 Additional OAuth providers (GitHub, Apple) - Phase 16
- 🛠️ Stripe webhook signatures - Phase 7

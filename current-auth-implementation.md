...
- **CORS**: `authApp` manually re-applies CORS headers to Auth.js responses so cross-origin cookie flows work (`apps/api/src/auth.ts`). API auth requires Bearer access tokens; Auth.js cookies alone are ignored by middleware.
...
- Auth.js CSRF checks are disabled (`skipCSRFCheck`) and Auth.js session/CSRF cookies are stripped from responses; the app relies solely on auth-core refresh CSRF + cookies.
- Sign-in callback (`maybeIssueAuthCoreSession` in `apps/api/src/auth.ts`):
  - Extracts the Auth.js-issued session token before stripping cookies from the response.
...
### 5) Personal Access Tokens (PATs) & API key management
...
### 6) First-party OAuth 2.0 (authorization code + PKCE)
...
- General auth endpoints (`/v1/auth/refresh|logout|sso/logout`): 10 req/min/IP; refresh endpoint elevated to 30 req/min/IP (`authRateLimitMiddleware`).

...
- When assigned with a task, before coding, create a file in .scope/tasks/ directory. in the file create a task-list. The task list should be numbered and use [ ] so we can later mark complete with [x]. At the end of each task add a short and sweet understandable acceptance criteria and/or sannity check if applicable. Once task list is completed wait for the user to fire off the tasks one item at a time.

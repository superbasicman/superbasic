...
if (!testEmail) {
  ...
  console.log('\nUsage:');
  console.log('  RESEND_API_KEY=re_xxx pnpm tsx tooling/scripts/test-resend.ts your-email@example.com');
  process.exit(1);
}
...
if (!apiKey || apiKey === 'your_resend_api_key_here') {
  ...
  console.log('\nSet it with:');
  console.log('  RESEND_API_KEY=re_xxx pnpm tsx tooling/scripts/test-resend.ts', testEmail);
  process.exit(1);
}
...
const magicLinkUrl = 'http://localhost:3000/v1/auth/callback/email?token=test_token_12345';

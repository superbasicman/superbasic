...
- [x] 1. Implement PAT logic in `auth-core`
  ...
  - Sanity check: `pnpm --filter @repo/auth-core exec vitest run src/__tests__/auth-service.test.ts` (passed)

- [x] 2. Refactor API Middleware (`apps/api/src/middleware/pat.ts`)
  ...
  - Sanity check: `pnpm --filter @repo/api exec vitest run src/middleware/__tests__/pat.test.ts` (passed)

- [x] 3. Migrate Token Management Routes (`apps/api/src/routes/v1/tokens`)
  ...
  - Sanity check: `pnpm --filter @repo/api exec vitest run "src/routes/v1/tokens/__tests__/*.test.ts"` (passed)

- [x] 4. Cleanup & Verification
  - Verify CLI/automation flows using the new PATs.
  - Ensure `pnpm deploy-check --full` passes. (passed)

...
- PATs still use legacy `api_keys` and bypass auth-core:
...
- Scope/role enforcement is effectively disabled for session users:
...
- OAuth/PKCE flow requires an existing access token and doesn’t use the IdP abstraction:
...
- `/v1/auth/token` still depends on an Auth.js session cookie instead of the unified IdP adapter:
...
- Postgres GUCs are set on the shared Prisma client, not per-request transactions:
...
- Refresh cookie hardening not at target:
